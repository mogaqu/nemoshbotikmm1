"""
╔══════════════════════════════════════════════════════════════════════════════╗
║          COIN HUNTER PRO  —  Roblox MM2 Bot  (2026 edition v2)              ║
    ...
    def __init__(self, use_dxcam: bool = True, window_name: str = 'Roblox'):
        ...
        if self._use_dxcam:
            try:
                ...
                print("[CAPTURE] dxcam активен (DirectX, HDR-совместим) ✓")
    ...
    def update(self, coin: Optional[dict], frame_shape: Tuple[int, int],
        ...
        try:
            ...
            if dist < self.COLLECT_DIST_PX:
                ...
                print(f"\n[💰] Монетка #{self.coins_collected} собрана! (ID={cid})")
                return "COLLECTED"
            ...
            return f"→ COIN#{cid}  Δx={error_x:+.0f}px  dist={dist:.0f}px{ghost_tag}"
...
class ConsoleHUD:
    ...
    def tick(self, status: str, coins: int, target: Optional[dict]):
        ...
        print(
            ...
            f"{Fore.YELLOW}💰 {coins:4d}{Style.RESET_ALL}  "
